...
import com.sparta.pracspring_blog.security.JwtTokenProvider;
...
public class UserController {
    ...
    @PostMapping("/signUp")
    public ResponseEntity<MsgResponseDto> signup(@Valid @RequestBody UserRequestDto userRequestDto, BindingResult bindingResult) {
        ...
        if(fieldErrors.size() > 0) { // 에러가 있는 경우
            ...
            // 5) 검증 실패시 응답
            return ResponseEntity.status(201).body(new MsgResponseDto("회원가입 실패", HttpStatus.CREATED.value()));
        }
